...
| `score`         | INT          | DEFAULT 1000      | Điểm số xếp hạng (ELO) hiện tại.           |
| `highest_score` | INT          | DEFAULT 1000      | Điểm số cao nhất từng đạt được.            |
...
| Cột (Column)  | Kiểu dữ liệu | Ràng buộc    | Mô tả                                             |
| :------------ | :----------- | :----------- | :------------------------------------------------ |
| `id`          | UUID         | PK           | Mã định danh duy nhất của bộ bài.                 |
...
| Cột (Column)  | Kiểu dữ liệu | Ràng buộc         | Mô tả                                          |
| :------------ | :----------- | :---------------- | :--------------------------------------------- |
| `id`          | UUID         | PK                | Mã định danh duy nhất của lá bài.              |
| `card_set_id` | UUID         | FK (card_sets.id) | Thuộc bộ bài nào.                              |
...
Hộp thư chung cho các tin nhắn báo động và lịch sử hệ thống.
...
| `type`        | VARCHAR(50)  | NOT NULL      | Loại: `system_alert`, `level_up`, `match_ended`.     |
| `payload`     | JSONB        |               | Dữ liệu động đi kèm.                                 |
...
Quản lý lời mời tham gia phòng chơi (tạm thời và có thời hạn).

| Cột (Column)  | Kiểu dữ liệu | Ràng buộc     | Mô tả                                                                    |
| :------------ | :----------- | :------------ | :----------------------------------------------------------------------- |
| `id`          | UUID         | PK            | Mã định danh duy nhất của lời mời.                                       |
| `sender_id`   | UUID         | FK (users.id) | Người gửi lời mời.                                                       |
| `receiver_id` | UUID         | FK (users.id) | Người nhận lời mời.                                                      |
| `room_code`   | VARCHAR(10)  | NOT NULL      | Mã phòng để tham gia.                                                    |
| `status`      | VARCHAR(20)  | NOT NULL      | Trạng thái: `pending`, `accepted`, `rejected`, `expired`.                |
| `expires_at`  | TIMESTAMP    | NOT NULL      | Thời điểm hết hạn lời mời.                                               |
| `created_at`  | TIMESTAMP    | NOT NULL      | Thời điểm tạo lời mời.                                                   |
