...
```json
{"type":1,"target":"SendCommand","arguments":[{"Event":"playcard","Data":{"cardCode":"Favor","targetUserId":"UUID_CUA_DANG_CHOI"}}]}
```
...
```json
{"type":1,"target":"SendCommand","arguments":[{"Event":"choosebombinsertposition","Data":{"position":3}}]}
```
...
```json
{"type":1,"target":"SendCommand","arguments":[{"Event":"requeststatesnapshot","Data":{}}]}
```

### Heartbeat (Giữ kết nối)
Gửi định kỳ nếu bị idle timeout.
```json
{"type":1,"target":"SendCommand","arguments":[{"Event":"heartbeat","Data":{}}]}
```
